...
- For this workflow, proceed through planned implementation phases directly without pausing to ask for confirmation or validation prompts, including during intermediate migration steps.
...
- For this codebase's bgfx migration, adapt the existing renderer abstraction rather than introducing a separately named `NiBgfxRenderer`; shaders should be stored as source files, with shader-cache support evaluated/reused from the DX11 path.
- For the renderer modernization, perform a full rewrite rather than a staged compatibility port: replace D3D11 and retire the deprecated DX9 and D3D10 renderer paths once bgfx works.