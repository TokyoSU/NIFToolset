...
#define DECORATION_PROJECTED_EFFECTS 2
...
SAMPLER2D(s_decorationFadeMask, 15);
...
uniform vec4 u_decorationFade;
uniform vec4 u_decorationParams;
...
vec4 sampleProjectedTexture(int slot, vec2 uv)
{
    ...
    return texture2D(s_projectedTexture1, uv);
}
...
vec4 sampleShadow2DStage(int stage, vec2 uv)
{
    ...
    // Stage 15 is the decoration fade mask and is never lent to shadows.
...
void main()
{
    ...
    // NiDecorationMaterial::DistanceAlpha. Distance is intentionally XY-only
    // to match the original node graph used for terrain decoration cells.
    if (u_decorationParams.y > 0.5)
    {
        vec3 diff = v_worldPos - u_cameraPosition.xyz;
        diff.z = 0.0;
        float distSqr = dot(diff, diff);
        float invOpacity = 0.0;
        if (distSqr > u_decorationFade.x)
            invOpacity = smoothstep(u_decorationFade.x, u_decorationFade.y, distSqr);
        else if (distSqr < u_decorationFade.w)
            invOpacity = 1.0 - smoothstep(u_decorationFade.z, u_decorationFade.w, distSqr);

        float noiseOpacity = texture2D(s_decorationFadeMask, v_texcoord01.xy).x;
        if (noiseOpacity - invOpacity < 0.0)
            discard;
    }
    ...
    for (int projectedIndex = 0; projectedIndex < DECORATION_PROJECTED_EFFECTS; ++projectedIndex)
    {
    ...
    if (u_mapParams[MAP_BASE].x > 0.5)
    {
        ...
        finalOpacity *= base.a;
        texDiffuse *= u_decorationParams.x;
    }
