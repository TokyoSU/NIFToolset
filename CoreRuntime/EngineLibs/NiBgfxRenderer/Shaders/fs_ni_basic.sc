...
// NiLightPrePass globals. u_lppParams: x=enabled/mode, y=L-buffer sampler
// stage for final passes, z/w reserved. DepthScale exactly mirrors
// g_fDepthScale from NiLPPViewRenderClick.
uniform vec4 u_lppParams;
uniform vec4 u_lppDepthScale;
uniform vec4 u_lppPosScale;
...
{
    if (stage == 0) return texture2D(s_baseTexture, uv);
    if (stage == 1) return texture2D(s_darkTexture, uv);
...
vec2 lppScreenUV(vec4 clipPos)
{
    float safeW = abs(clipPos.w) > 1e-6 ? clipPos.w : 1.0;
    vec2 ndc = clipPos.xy / safeW;
    vec2 uv = vec2(ndc.x, -ndc.y) * 0.5 + vec2(0.5, 0.5);
    return uv + u_lppDepthScale.zw;
}

vec4 lppEncodeGBuffer(vec3 normal, vec3 worldPos, float specularPower)
{
    float viewZ = abs(dot(worldPos - u_cameraPosition.xyz,
        safeNormalize(u_cameraDirection.xyz, vec3(0.0, 0.0, 1.0))));
    float packedDepth = (viewZ / max(u_lppDepthScale.x, 1e-6)) - u_lppDepthScale.y;
    packedDepth = min(max(packedDepth, 0.0), 255.0);

    vec3 n = safeNormalize(normal, vec3(0.0, 0.0, 1.0));
    float p = sqrt(max(n.z * 8.0 + 8.0, 1e-6));
    vec2 scaledNorm = n.xy / p + vec2(0.5, 0.5);
    return vec4(scaledNorm, saturate(specularPower / 255.0), packedDepth / 255.0);
}

vec4 sampleLppStage(int stage, vec2 uv)
{
    return sampleShadow2DStage(stage, uv);
}

vec3 lppDecodeNormal(vec4 gbuffer)
{
    vec2 fenc = gbuffer.xy * 4.0 - 2.0;
    float f = dot(fenc, fenc);
    float g = sqrt(max(1.0 - f * 0.25, 0.0));
    return safeNormalize(vec3(fenc * g, 1.0 - f * 0.5), vec3(0.0, 0.0, 1.0));
}

vec3 lppDecodeWorldPosition(vec4 gbuffer, vec2 screenUV)
{
    float depth = gbuffer.w * 255.0;
    depth = (depth + u_lppDepthScale.y) * u_lppDepthScale.x;
    vec2 coord = screenUV - vec2(0.5, 0.5);
    float perspectiveCorrection = dot(vec2(depth, 1.0), u_lppProjectionSwitch.xy);
    vec3 viewPos = vec3(
        perspectiveCorrection * u_lppPosScale.x * coord.x,
        -perspectiveCorrection * u_lppPosScale.y * coord.y,
        depth);
    return u_cameraPosition.xyz +
        u_lppCameraRight.xyz * viewPos.x +
        u_lppCameraUp.xyz * viewPos.y +
        safeNormalize(u_cameraDirection.xyz, vec3(0.0, 0.0, 1.0)) * viewPos.z;
}
...
void main()
{
...
#ifdef LPP_LIGHT
    // The light-volume vertex shader stores projected position in v_texcoord67.
    vec2 lppUV = lppScreenUV(v_texcoord67);
    vec4 gbuffer = texture2D(s_baseTexture, lppUV);
    vec3 lppNormal = lppDecodeNormal(gbuffer);
    vec3 lppWorldPos = lppDecodeWorldPosition(gbuffer, lppUV);
    vec3 lppView = safeNormalize(u_cameraPosition.xyz - lppWorldPos,
        vec3(0.0, 0.0, 1.0));
    float lppSpecPower = max(gbuffer.z * 255.0, 1.0);

    vec3 lightDiffuse = vec3(0.0, 0.0, 0.0);
    vec3 lightSpecular = vec3(0.0, 0.0, 0.0);
    vec3 lightAmbient = vec3(0.0, 0.0, 0.0);
    int lppLightCount = int(u_lightCount.x + 0.5);
    if (lppLightCount > 0)
    {
        float type = u_lightPositionType[0].w;
        vec3 L;
        float attenuation = 1.0;
        if (type < 0.5)
        {
            L = safeNormalize(-u_lightDirectionRange[0].xyz,
                vec3(0.0, 0.0, 1.0));
        }
        else
        {
            vec3 toLight = u_lightPositionType[0].xyz - lppWorldPos;
            float distanceToLight = length(toLight);
            L = distanceToLight > 1e-6 ? toLight / distanceToLight : lppNormal;
            float range = u_lightDirectionRange[0].w;
            if (range > 0.0)
            {
                attenuation = saturate(1.0 - distanceToLight / range);
                attenuation = pow(attenuation, max(u_lightAmbientFalloff[0].w, 1.0));
            }
            if (type > 1.5)
            {
                vec3 spotDir = safeNormalize(u_lightDirectionRange[0].xyz,
                    vec3(0.0, 0.0, 1.0));
                float cone = dot(-L, spotDir);
                float innerCos = u_lightSpotParams[0].x;
                float outerCos = u_lightSpotParams[0].y;
                float coneRange = max(innerCos - outerCos, 1e-5);
                float spot = saturate((cone - outerCos) / coneRange);
                attenuation *= pow(spot, max(u_lightSpotParams[0].z, 1.0));
            }
        }

        attenuation *= evaluateLightShadow(0, type, lppWorldPos);
        float dimmer = u_lightDiffuseDimmer[0].w;
        float ndotl = max(dot(lppNormal, L), 0.0);
        lightAmbient = u_lightAmbientFalloff[0].rgb * dimmer * attenuation;
        lightDiffuse = u_lightDiffuseDimmer[0].rgb * dimmer * ndotl * attenuation;
        if (ndotl > 0.0)
        {
            vec3 H = safeNormalize(L + lppView, lppView);
            float spec = pow(max(dot(lppNormal, H), 0.0), lppSpecPower);
            lightSpecular = u_lightSpecularSpot[0].rgb * dimmer * spec * attenuation;
        }
    }

    vec3 lppAccum = lightDiffuse + lightAmbient;
    float lppSpecMean = dot(lightSpecular, vec3(1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0));
    gl_FragColor = vec4(lppAccum, lppSpecMean);
    return;
#endif
...
#ifdef LPP_FINAL
    vec4 projectedPosition = mul(u_viewProj, vec4(v_worldPos, 1.0));
    vec2 lppUV = lppScreenUV(projectedPosition);
    vec4 lppLighting = sampleLppStage(int(u_lppParams.y + 0.5), lppUV);
    // ReconstructLightingFromTexture feeds the L-buffer RGB back as the
    // diffuse-light accumulator and RGB*alpha as specular-light accumulator.
    vec3 diffuseCoeff = materialEmissive + materialDiffuse * lppLighting.rgb;
    vec3 specularCoeff = specularEnabled ?
        u_materialSpecular.rgb * (lppLighting.rgb * lppLighting.a) * glossFactor :
        vec3(0.0, 0.0, 0.0);
#else
    ...
    // Reproduce NiStandardMaterial's texture accumulator order. Dark maps and
    // projected lights feed the diffuse texture accumulator before the base
    // map; detail/decal maps follow it; projected shadows multiply the result.
    vec3 texDiffuse = vec3(1.0, 1.0, 1.0);
    bool texDiffuseInitialized = false;

    if (u_mapParams[MAP_DARK].x > 0.5)
    {
        texDiffuse = texture2D(s_darkTexture, mapUV(MAP_DARK, v_texcoord01, v_texcoord23, v_texcoord45, v_texcoord67)).rgb;
        texDiffuseInitialized = true;
    }

    for (int projectedIndex = 0; projectedIndex < MAX_PROJECTED_EFFECTS; ++projectedIndex)
    {
        if (u_projectedParams[projectedIndex].x <= 0.5 ||
            u_projectedParams[projectedIndex].y > 0.5)
        {
            continue;
        }

        vec3 projectedColor = sampleProjectedEffect(projectedIndex, v_worldPos).rgb;
        if (u_projectedParams[projectedIndex].w > 0.5)
            projectedColor *= projectedClipScalar(projectedIndex, v_worldPos, false);

        if (texDiffuseInitialized)
            texDiffuse += projectedColor;
        else
        {
            texDiffuse = projectedColor;
            texDiffuseInitialized = true;
    ...
    float finalOpacity = replaceMode ? 1.0 : materialOpacity;
    if (u_mapParams[MAP_BASE].x > 0.5)
    {
        vec4 base = texture2D(s_baseTexture, baseUV);
        if (texDiffuseInitialized)
            texDiffuse *= base.rgb;
        else
        {
    ...
    if (!texDiffuseInitialized)
        texDiffuse = vec3(1.0, 1.0, 1.0);

    if (u_mapParams[MAP_DETAIL].x > 0.5)
        texDiffuse *= texture2D(s_detailTexture, mapUV(MAP_DETAIL, v_texcoord01, v_texcoord23, v_texcoord45, v_texcoord67)).rgb * 2.0;

    if (u_mapParams[MAP_DECAL0].x > 0.5)
    {
        vec4 decal = texture2D(s_decalTexture0, mapUV(MAP_DECAL0, v_texcoord01, v_texcoord23, v_texcoord45, v_texcoord67));
        texDiffuse = mix(texDiffuse, decal.rgb, decal.a);
    }
    if (u_mapParams[MAP_DECAL1].x > 0.5)
    {
        vec4 decal = texture2D(s_decalTexture1, mapUV(MAP_DECAL1, v_texcoord01, v_texcoord23, v_texcoord45, v_texcoord67));
        texDiffuse = mix(texDiffuse, decal.rgb, decal.a);
    }
    if (u_mapParams[MAP_DECAL2].x > 0.5)
    {
        vec4 decal = texture2D(s_decalTexture2, mapUV(MAP_DECAL2, v_texcoord01, v_texcoord23, v_texcoord45, v_texcoord67));
        texDiffuse = mix(texDiffuse, decal.rgb, decal.a);
    }
    ...
    for (int projectedIndex = 0; projectedIndex < MAX_PROJECTED_EFFECTS; ++projectedIndex)
    {
        ...
        vec3 shadowColor = sampleProjectedEffect(projectedIndex, v_worldPos).rgb;
        float clipScalar = projectedClipScalar(projectedIndex, v_worldPos, true);
        texDiffuse *= saturate(shadowColor + vec3(clipScalar, clipScalar, clipScalar));
    }
